...
fn solve(values: &[usize], n: usize) -> usize {
    ...
    values
        .iter()
        .map(|v| fibo_alt(n + GESTATION_PERIOD - v - 1, &mut cache))
        .sum()
}

fn fibo_alt(n: usize, cache: &mut HashMap<usize, usize>) -> usize {
    match cache.get(&n) {
        Some(v) => *v,
        None => {
            let v = if n < GESTATION_PERIOD {
                ...
                fibo_alt(n - GESTATION_PERIOD, cache) + fibo_alt(n - REPRODUCTION_RATE, cache)
            };
            cache.insert(n, v);
            v
        }
