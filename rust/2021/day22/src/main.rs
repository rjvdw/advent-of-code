...
fn execute_reboot_sequence(cuboids: &[Cuboid]) -> (usize, usize) {
    let initialization_region = Cuboid {
        ...
        x_range: Range::new(-50, 50),
        y_range: Range::new(-50, 50),
        z_range: Range::new(-50, 50),
    };

    let mut init_done = false;
    let mut init_count = 0;
    let mut on_regions = vec![];

    for cuboid in cuboids {
        // assumption: all cuboids for the initialization sequence are at the start
        if !init_done && !cuboid.fits_within(&initialization_region) {
            init_done = true;
            init_count = count_cubes(&on_regions);
        }
...
mod tests {
    ...
    #[test]
    fn test_execute_reboot_sequence_1() {
        let cuboids = vec![
        ...
        assert_eq!(execute_reboot_sequence(&cuboids), (39, 39));
    }

    #[test]
    fn test_execute_reboot_sequence_2() {
        let cuboids = vec![
        ...
        assert_eq!(execute_reboot_sequence(&cuboids), (590784, 39769202357779));
    }

    #[test]
    fn test_execute_reboot_sequence_3() {
        let cuboids = vec![
        ...
        assert_eq!(
            execute_reboot_sequence(&cuboids),
