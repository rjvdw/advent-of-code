...
fn main() -> MainResult {
    ...
    let button_presses = modules.press_button_until_module_is_activated(&args.sand_module);
    println!(
        "It takes {button_presses} button before module {} is activated by a single low pulse",
        args.sand_module
    );
